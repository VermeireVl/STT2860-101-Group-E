...
author: "Viktor Vermeire, Ayden Matocha, Nolan Boucher, Ethan Carter"
...
Go to the [description](https://stat-jet-asu.github.io/Datasets/InstructorDescriptions/NFL2016.html) NFL Players 2016 Dataset and read the brief summary and variable definitions. Use the URL below to read the data into `R`. Save the dataset as an object named `NFL`.
...
**ANSWER:** I would agree because since they have a higher muscle mass it raises their BMI and they could be considered unhealthy despite being great athletes. BFP would be a much better measurement.
...
```{r playtimeboxplot}
ggplot(NFL, aes(x = Playtime)) + geom_bar(fill = "#0073C2FF", color = "white") + scale_y_continuous(breaks = seq(0,max(table(NFL$Playtime)) + 100, by = 100)) + labs(title = "Distribution of NFL Player Playtime", subtitle = "Playtime levels across all recorded players",
x = "Playtime Category", y = "Number of Players") + theme_light(base_size = 14) + theme(legend.position = "none")
```
...
```{r ageviolins}
ggplot(NFL, aes(x = Age, y = Side)) + geom_violin(fill = "#B3CDE3", color = "gray40", alpha = 0.8) + geom_boxplot(width = 0.15, fill = "#005B96", color = "red", outlier.size = 2.5) + labs( title = "Distribution of NFL Player Ages by Field Side", subtitle = "Comparison of player ages across offense, defense, and special teams", x = "Age (years)", y = "Side of Play") + theme_light(base_size = 14)
```
...
```{r yearsbeeswarm}
ggplot(NFL, aes(x = Side, y = Years, fill = Playtime)) + geom_beeswarm(cex = 1.2, size = 3, shape = 21, color = "black") + scale_fill_viridis_d(option = "plasma") + labs( title = "NFL Player Career Length by Field Side", subtitle = "Each point represents one player, color-coded by playtime level", x = "Side of Play", y = "Career Length (Years)", fill = "Playtime Level") + theme_light(base_size = 14) + theme(legend.position = "bottom")
``` 
...
**ANSWER:** The plots show that most NFL players have short careers with most of them under 10 years. This includes active players in their mid-20s to early 30s. While playtime and age vary across both sides of the ball, the majority of players see shorter playtime and careers. These plots however, don't do a good job explaining why careers are short. This could be due to injuries or the nature of football. To improve on this they could show how career length changes over time due to injuries.** 
